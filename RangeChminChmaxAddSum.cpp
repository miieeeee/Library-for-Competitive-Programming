...
struct RangeChminChmaxAddSum{
    ...
    SegtreeBeats<S, op, e, F, mapping, composition, id> seg;   
    public:
    RangeChminChmaxAddSum(int sz):seg(sz){}
    RangeChminChmaxAddSum(vector<S> a):seg(a){}
    void set(int i, S x){
        seg.set(i, x);
    }
    void set(int i, ll x){
    ...
    void range_chmin(int l, int r, ll x){
        seg.apply(l, r, F(-LINF, x, 0));
    }
    void range_chmax(int l, int r, ll x){
        seg.apply(l, r, F(x, LINF, 0));
    }
    void range_add(int l, int r, ll x){
        seg.apply(l, r, F(-LINF, LINF, x));
    }
    S prod(int l, int r){
        return seg.prod(l, r);
    }
    S all_prod(int l, int r){
        return seg.all_prod();
    }
    ll sum(int l, int r){
        return seg.prod(l, r).sum;
    }
    ll all_sum(int l, int r){
...
int main(){
    ...
    while(q--){
        ...
        if(t == 3){
            ...
            ll x;
            cin >> x;
            if(t == 0) seg.range_chmin(l, r, x);
            if(t == 1) seg.range_chmax(l, r, x);
            if(t == 2) seg.range_add(l, r, x);
        }
    }
}