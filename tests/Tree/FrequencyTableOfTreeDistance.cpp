...
#include "Tree/CentroidDecomposition/CentroidDecomposition.hpp"
...
void main2(){
    ...
    auto [cr, cp, ct] = cd.build();

    vc<bool> used(n, false);
    vll ans(n, 0);
    auto depth = [&](auto self, int v, int pv, vll &data, int dep) -> void {
        if(data.size() <= dep) data.resize(dep+1, 0);
        data[dep]++;
        for(auto nv : g[v]){
            if(used[nv] || nv == pv) continue;
            self(self, nv, v, data, dep+1);
        }
    };

    auto solve = [&](auto self, int v, int pv) -> void {
        used[v] = true;
        vvll cv;
        int max_depth = 0;
        for(auto nv : g[v]) {
            if(used[nv]) continue;
            vll c(1, 0);
            depth(depth, nv, v, c, 1);
            chmax(max_depth, c.size()-1);
            cv.push_back(c);
        }
        for(auto nv : ct[v]) {
            if(used[nv] || nv == pv) continue;
        ...
        vll tot(max_depth+1, 0);
        for(auto c : cv) {
            rep(i, c.size()) {
                tot[i] += c[i];
                ans[i] += 2*ll(c[i]);
            }
            auto conv = atcoder::convolution_ll(c, c);
            rep(i, conv.size()) {
                ans[i] -= conv[i];
            }
        }
        auto conv = atcoder::convolution_ll(tot, tot);
        rep(i, conv.size()) ans[i] += conv[i];
        used[v] = false;
    };

    solve(solve, cr, -1);
    rep(i, n-1) cout << ans[i+1]/2 << " ";
    cout << el;
}