...
S op1(S l, S r) { return S{l.a*r.a, l.b*r.a + r.b}; }
S op2(S l, S r) { return S{r.a*l.a, r.b*l.a + l.b}; }
S e() { return S{1, 0}; }
...
void main2(){
    ...
    rep(i, n) {
        seg1.set(hld.index(i), S{a[i], b[i]});
        seg2.set(hld.index(i), S{a[i], b[i]});
    }
    ...
    while(q--) {
        ...
        if(t == 0) {
            ...
            a[p] = c;
            b[p] = d;
            seg1.set(hld.index(p), S{a[p], b[p]});
            seg2.set(hld.index(p), S{a[p], b[p]});
        }
        if(t == 1) {
            int u, v, x;
            cin >> u >> v >> x;
            S sum = e();
            auto f = [&](int l, int r, bool reverse) -> void {
                if(reverse) {
                    sum = op1(sum, seg2.prod(l, r));
                }
                else {
                    sum = op1(sum, seg1.prod(l, r));
                }
            };

            hld.path_query(u, v, false, f);
            cout << (sum.a*x + sum.b).val() << el;
        }
    }
}