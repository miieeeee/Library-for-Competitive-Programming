...
#include "Tree/CentroidDecomposition/CentroidDecompositionBinaryTree.hpp"
...
void main2(){
    ...
    CentroidDecompositionBinaryTree cdbt(g);
    cdbt.build();

    int m = cdbt.get_vec_count();
    vector<atcoder::segtree<ll, op, e>> segs(m);
    rep(i, m) {
        auto vec = cdbt.get_vec(i);
        segs[i] = atcoder::segtree<ll, op, e>(vec.size());
        rep(j, vec.size()) {
            segs[i].set(j, a[vec[j]]);
        }
    }
    ...
    while(q--) {
        ...
        if(t == 0) {
            ...
            cin >> p >> x;
            a[p] += x; 
            for(auto [vid, idx] : cdbt.get_indexes(p)) {
                segs[vid].set(idx, a[p]);
            }
        }
        else {
            int p, l, r;
            cin >> p >> l >> r;
            ll sum = 0;
            for(auto [vid, lp, rp] : cdbt.get_contour_query_range(p, l, r)) {
                sum += segs[vid].prod(lp, rp);
            }
