...
struct DP
{
    using Path = pair<mint, mint>;
    using Point = mint;

    static Path vertex(int i) { return {1, a[i]}; }
    static Path add_vertex(Point p, int i) { return {p, a[i]}; }
    static Point add_edge(Path p) { return p.second; }
    static Point rake(Point l, Point r) { return l*r; }
    static Path compress(Path p, Path c) { return {p.first*c.first, p.first*c.second + p.second}; }
};
void main2(){
    int n, q;
    cin >> n >> q;
